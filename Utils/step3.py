...
def install_nephelees(db, path, step1data: dict):
    db.execute("""CREATE DATABASE IF NOT EXISTS cantina_nephelees""")
    db.execute("CREATE TABLE IF NOT EXISTS cantina_nephelees.file_sharing(id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, "
               "file_name TEXT, file_owner text, file_short_name TEXT, login_to_show BOOL DEFAULT 1, password TEXT,"
               "date TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)")

    system(f"cd {path} && git clone https://github.com/Cantina-Org/Nephelees.git")
    ...
    with open(path+'/Nephelees/config.json', "w") as outfile:
